...
class controlador:
    ...
    def Obtener_Fecha_Servidor(self):
        ...
        return fecha_consulta,edo_consulta
    
    
    def obtener_clase_alumno(self):
        "Obtener la Clase del Alumno"
        clase = ""
        mensaje = ""
        # Obtenemos el Dia y la Hora
        consulta,edo_consulta = self.modelo.hora_sistema()
        if edo_consulta == "SUCCESS":
            # Obtenemos la Fecha y Hora en la cual se hizo click en el Boton de Registrar Asistencia
            fecha = datetime.datetime.strptime(str (consulta[0][0]),"%Y-%m-%d %H:%M:%S")
            ...
            if edo_consulta == "SUCCESS":
                if consulta:
                    for gpo in consulta:
                        ...
                        consulta2,edo_consulta2 = self.modelo.clase_horario(gpo[0],dia,hora)
            ...
            print "Fallo Consulta a la Hora del Sistema"
            clase = ""
            mensaje = edo_consulta
            return clase,mensaje
    
    def obtener_asist_alum(self):
        "Se Obtiene la Asistencia del Alumno para esa Clase"
        mensaje = ""
        asistencia = ""
        fecha_consulta,edo_consulta = self.Obtener_Fecha_Servidor()
        if edo_consulta == "SUCCESS":
            # Buscamos si el alumno ya tiene Asistencia para esta Materia en este dia
            # True si ya tiene una Asistencia
    ...
    def registrar_asistencia(self):
        ...
        consulta,edo_consulta = self.modelo.hora_sistema()
        if edo_consulta == "SUCCESS":
            delta = datetime.datetime.strptime(str (consulta[0][0]),"%Y-%m-%d %H:%M:%S")
            hora = int(delta.strftime('%H'))
            minuto = int(delta.strftime('%M'))
                    ...
                    if consulta:
                        ...
                        if self.band_clase == True:
                            ...
                            if edo_consulta == "SUCCESS_QUERY_ATTENDANCE":
                                ...
                                consulta2,edo_consulta2 = self.obtener_asist_alum()
                                if edo_consulta2 == "SUCCESS":
